...
async function waitForResponse(timeoutMs: number = 5000): Promise<any> {
...
async function executeDiskCleanKitCommand(operation: string, params: Record<string, string> = {}): Promise<any> {
...
- Last Cleaning: ${stats.lastCleaningDate || "Never"}`;
      ...
      case "get_large_files": {
        const minSizeMb = (args as any)?.min_size_mb || 100;
